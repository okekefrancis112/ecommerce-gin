...
func (c *authDatabase) SaveOtpSession(ctx context.Context, otpSession domain.OtpSession) error {
	...
	query := `INSERT INTO otp_sessions (otp_id, user_id, phone, email, expire_at)
	VALUES ($1, $2, $3, $4, $5)`
	err := c.DB.Exec(query, otpSession.OtpID, otpSession.UserID, otpSession.Phone, otpSession.Email, otpSession.ExpireAt).Error
	return err
}
