...
import (
	...
	"github.com/okekefrancis112/ecommerce-gin/pkg/repository/interfaces"
	"github.com/okekefrancis112/ecommerce-gin/pkg/service/otp"
	"github.com/okekefrancis112/ecommerce-gin/pkg/service/token"
	service "github.com/okekefrancis112/ecommerce-gin/pkg/usecase/interfaces"
	"github.com/okekefrancis112/ecommerce-gin/pkg/utils"
)
...
func (c *authUseCase) UserLoginOtpSend(ctx context.Context, loginDetails request.OTPLogin) (string, error) {
	...
	go func() {
		...
		_, err := c.optAuth.SendOtp(user.Email)
		if err != nil {
...
func (c *authUseCase) LoginOtpVerify(ctx context.Context, otpVerifyDetails request.OTPVerify) (uint, error) {
	...
	// valid, err := c.optAuth.VerifyOtp(countryCode+otpSession.Email, otpVerifyDetails.Otp)
	valid, err := c.optAuth.VerifyOtp(otpSession.Email, otpVerifyDetails.Otp)
	if err != nil {
...
func (c *authUseCase) UserSignUp(ctx context.Context, signUpDetails domain.User) (string, error) {
	...
	go func() {
		...
		_, err := c.optAuth.SendOtp(signUpDetails.Email)
		if err != nil {
			errChan <- fmt.Errorf("failed to send otp \nerrors:%v", err.Error())
		}
	}()
